...
public class SHAEncryptService {
    ...
    private String encryptPwdWithSalt(String pwd, String salt) {
        ...
        try {
            ...
            String hexStrSalt = this.bytes2Hex(salt.getBytes(Constant.CHAR_SET));
    ...
    private String getSaltFromEncryptedPwd(String encPwd) {
        ...
        try {
            ...
            int hexSaltLength = Constant.SALT_LENGTH * 2;
            ...
            ret = new String(this.hex2Bytes(hexSalt), Constant.CHAR_SET);
        } catch (Exception e) {
            ret = "";
            e.printStackTrace();
        }
    ...
    private String genSalt() {
        ...
        Random r = new Random();
        int temp = 0;
        for (int i = 0; i < Constant.SALT_LENGTH; i++) {
            temp = r.nextInt(Constant.SALT_ARR.length);
            result.append(Constant.SALT_ARR[temp]);
        }
    ...
    private String joinSaltAndPwd(String salt, String pwd) {
        if (salt.length() != Constant.SALT_LENGTH) {
    ...
    private String encodeJoinStr(String joinStr) {
        ...
        for (int i = 0; i < Constant.ENCODE_COUNT; i++) {
    ...
    private String encrypt(String src) {
        ...
        // 源字符串转换成byte数组
        try {
            byte[] btSource = src.getBytes(Constant.CHAR_SET);
            // 此处选择了SHA-256加密算法,实际可选的算法有"MD2、MD5、SHA-1、SHA-256、SHA-384、SHA-512"
            MessageDigest md = MessageDigest.getInstance(Constant.ENCODE_TYPE);
            md.reset();
            md.update(btSource);
